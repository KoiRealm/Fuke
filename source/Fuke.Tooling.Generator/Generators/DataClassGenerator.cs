...
public static class DataClassGenerator
{
    ...
    private static void WritePropertyDeclaration(DataClassWriter writer, Property property)
    {
        ...
        var newModifier = property.HidesBaseMember ? "new " : null;

        writer
            .WriteLine($"/// <summary>{property.Help}</summary>")
            .WriteLine($"{attributes.JoinSpace()} public {newModifier}{type.External} {property.Name} => Get<{type.Internal}>(() => {property.Name});");
