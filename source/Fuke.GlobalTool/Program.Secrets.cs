...
{
    private const string SaveAndExit = "<保存并退出>";
    private const string DiscardAndExit = "<放弃并退出>";
    private const string DeletePasswordAndExit = "<删除密码并退出>";
    ...
    public static int Secrets(string[] args, [CanBeNull] AbsolutePath rootDirectory, [CanBeNull] AbsolutePath buildScript)
    {
        var secretParameters = CompletionUtility.GetItemsFromSchema(
                GetBuildSchemaFile(rootDirectory.NotNull("未找到根目录")),
                filter: x => x.Value.TryGetProperty("default", out _))
            .Select(x => x.Key).ToList();
        if (secretParameters.Count == 0)
        {
            Host.Information($"没有标记为 {nameof(SecretAttribute)} 的参数");
            return 0;
        }
        ...
        {
            if (generatedPassword || PromptForConfirmation($"是否将密码保存到钥匙串？（关联目录：“{rootDirectory}”）"))
