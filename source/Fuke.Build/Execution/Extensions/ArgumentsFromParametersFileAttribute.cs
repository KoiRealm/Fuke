...
public class ArgumentsFromParametersFileAttribute : BuildExtensionAttributeBase, IOnBuildCreated
{
    public void OnBuildCreated(IReadOnlyCollection<ExecutableTarget> executableTargets)
    {
        ...
        if (!Constants.GetFukeDirectory(FukeBuild.RootDirectory).DirectoryExists())
            return;
        ...
        //         ? EnvironmentInfo.WorkingDirectory.GetUnixRelativePathTo(FukeBuild.RootDirectory / value)
        //         : value;

        var parameterMembers = ValueInjectionUtility.GetParameterMembers(Build.GetType(), includeUnlisted: true);
        var jobjectsAndProfiles = new[] { (File: Constants.GetDefaultParametersFile(FukeBuild.RootDirectory), Profile: Constants.DefaultProfileName) }
            .Where(x => File.Exists(x.File))
            .Concat(FukeBuild.LoadedLocalProfiles.Select(x => (File: Constants.GetParametersProfileFile(FukeBuild.RootDirectory, x), Profile: x)))
        ...
        ParameterService.Instance.ArgumentsFromFilesService = (parameter, destinationType) =>
        {
            ...
            if (typeof(IAbsolutePathHolder).IsAssignableFrom(scalarType))
                return property.Value.ToObject<string>().Apply(x => !PathConstruction.HasPathRoot(x) ? FukeBuild.RootDirectory / x : (AbsolutePath)x);
