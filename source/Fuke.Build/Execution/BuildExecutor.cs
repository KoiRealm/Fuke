...
{
    // NOTE: no IFukeBuild because of BuildAttemptFile + WriteTarget
    private static AbsolutePath BuildAttemptFile => Constants.GetBuildAttemptFile(FukeBuild.RootDirectory);

    public static void Execute(FukeBuild build, [CanBeNull] IReadOnlyCollection<string> skippedTargets)
    {
    ...
    private static IReadOnlyCollection<string> UpdateInvocationHash(FukeBuild build)
    {
    ...
    private static bool CheckConditions(IFukeBuild build, ExecutableTarget target, IEnumerable<(string Text, Func<bool> Delegate)> conditions)
    {
    ...
    private static void MarkTargetSkipped(IFukeBuild build, ExecutableTarget target, string reason = null)
    {
