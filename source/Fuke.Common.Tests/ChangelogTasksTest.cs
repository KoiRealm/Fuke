...
public class ChangelogTasksTest
{
    ...
    private static AbsolutePath PathToChangelogReferenceFiles => RootDirectory / "source" / "Fuke.Common.Tests" / "ChangelogReferenceFiles";

    [Theory]
    [MemberData(nameof(GetAllChangelogReference_1_0_0_Files))]
    [MemberData(nameof(GetAllChangelogReference_FUKE_Files))]
    ...
    [UsedImplicitly]
    public static IEnumerable<object[]> GetAllChangelogReference_1_0_0_Files()
        => PathToChangelogReferenceFiles.GlobFiles("changelog_reference_1.0.0*.md").Select(x => new object[] { x });

    [UsedImplicitly]
    public static IEnumerable<object[]> GetAllChangelogReference_FUKE_Files()
        => PathToChangelogReferenceFiles.GlobFiles("changelog_reference_FUKE*.md").Select(x => new object[] { x });
}
